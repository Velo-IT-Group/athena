...
const { notifyWorker } = require(Runtime.getFunctions()['helpers/twilio/call_operations'].path);
const { getTicket } = require(Runtime.getFunctions()['helpers/manage/ticket_functions'].path);
const { syncTaskrouterWorkers, syncTaskrouterTasks } = require(Runtime.getFunctions()[
...
export const handler: ServerlessFunctionSignature = async function (
	...
	try {
		...
		if ((eventType === 'reservation.accepted' || eventType === 'reservation.created') && requestDict.TaskSid) {
			const parsedAttributes = JSON.parse(requestDict.TaskAttributes ?? '');
		...
		if (eventType?.startsWith('task.') || eventType?.startsWith('reservation.')) {
			await syncTaskrouterTasks({ client, syncService });
		}
		if (event.EventType?.startsWith('worker.') || event.EventType?.startsWith('task.')) {
			const queueStatusDocument = syncService.documents('Queue Status');
			const tasks = await syncService.syncMaps('SyncTaskRouterTasks').syncMapItems.list();
			...
			if (event.EventType === 'task.created') {
				attributes.taskType === 'voicemail' ? (doc.voicemails_in_queue += 1) : (doc.calls_in_queue += 1);
	...
	if (event.EventType === 'task-queue.entered' && event.WorkflowName === 'Voicemail' && event.TaskAttributes) {
		try {
			...
			if (ticket.board?.id !== 30 || ticket.status?.id !== 530) {
				await client.taskrouter.v1.workspaces(context.TWILIO_WORKSPACE_SID!).tasks(taskSid!).remove();
			...
			const workers = await client.taskrouter.v1.workspaces(context.TWILIO_WORKSPACE_SID!).workers.list({
	...
	if (event.EventType === 'task-queue.entered' && event.TaskQueueName === 'Voicemail') {
