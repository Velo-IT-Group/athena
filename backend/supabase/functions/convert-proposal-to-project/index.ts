...
Deno.serve(async (req: Request) => {
  const { proposalId, versionId }: reqPayload = await req.json();
...
export const throttledCreateManageProduct = asyncThrottle(
  async (product: any) => await createManageProduct(product),
  {
    wait: 200,
  },
);

export const throttledCreateProjectPhase = asyncThrottle(
  async (params: { projectId: number; phase: any }) =>
    await createProjectPhase(params.projectId, params.phase),
  {
    wait: 200,
  },
);
...
const throttledCreateProjectTicket = asyncThrottle(
  async (params: { phaseId: number; ticket: any }) =>
    await createProjectTicket(params.phaseId, params.ticket),
  {
    wait: 200,
  },
);
...
const throttledCreateProjectTask = asyncThrottle(
  async (params: { ticketId: number; task: any }) =>
    await createProjectTask(params.ticketId, params.task),
  {
    wait: 200,
  },
);
...
export const createManageProduct = async (product: any) => {
...
export const convertOpportunityToProject = async (id: number, data: any) => {
  const body = JSON.stringify(data);

  const response = await fetch(
    `${url}/sales/opportunities/${id}/convertToProject`,
    {
      method: "POST",
      body,
      headers,
  ...
  if (!response.ok) {
    throw new Error(
      "Error converting opportunity to project: \n\n" +
        JSON.stringify(await response.json()),
    );
  }
...
export const createProjectPhase = async (projectId: number, phase: any) => {
...
export const createProjectTicket = async (phaseId: number, ticket: any) => {
...
export const createProjectTask = async (
  ...
  const response = await fetch(
    `${url}/project/tickets/${ticketId}/tasks`,
    config,
  );
