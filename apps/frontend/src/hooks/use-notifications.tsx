...
const useNotifications = () => {
	...
	const { data: notifications } = useQuery(query);

	useEffect(() => {
		if (!notifications || ref.current) return;
		...
		const realtime = supabase.realtime
			...
			.on(
				...
				(payloadData) => {
					const payload =
						payloadData as PostgresChangeEvent<AppNotification>;
	...
	const markAllNotificationsAsRead = useMutation({
		...
		onMutate: () => {
			...
			const updatedNotifications = unreadNotifications?.map(
				(notification) => ({
					...notification,
					is_read: true,
				})
			...
			return {
				oldItems: unreadNotifications,
				newItems: updateNotification,
			};
		},
	...
	const markNotificationAsRead = useMutation({
		...
		onMutate: (id) => {
			...
			const notifications = queryClient
				.getQueryData<AppNotification[]>(queryKey)
				?.map((notification) => ({
					...notification,
					is_read: true,
				}));
