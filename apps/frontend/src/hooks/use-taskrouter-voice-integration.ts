...
export const useTaskRouterVoiceIntegration = ({
	...
	const isVoiceReservation = useCallback((reservation: Reservation) => {
		return (
			reservation.task?.taskChannelUniqueName?.toLowerCase() === "voice" ||
	...
	const findRelatedCall = useCallback(
		(reservation: Reservation) => {
			...
			const relatedCall = activeCalls.find(
				(call) => call.parameters.CallSid === workerCallSid,
			);

			if (relatedCall) {
				console.log(
					"Found related call:",
					relatedCall.parameters.CallSid,
					"for reservation:",
					reservation.sid,
				);
			} else {
				console.log(
					"No related call found for reservation:",
					reservation.sid,
					"looking for CallSid:",
					workerCallSid,
				);
	...
	const findPendingVoiceReservationForCall = useCallback(
		(call: Call) => {
	...
	const conferenceReservation = useCallback(
		async (reservationSid: string) => {
			if (!worker) {
			...
			try {
				...
				console.log(
					"Task attributes before conference:",
				...
				setTimeout(() => {
					...
					if (updatedReservation) {
						console.log(
							"Updated task attributes after conference:",
							updatedReservation.task.attributes,
						);
					}
				}, 1000);
			} catch (error) {
				console.error(
					"Failed to conference reservation:",
					reservationSid,
					error,
				);
			}
	...
	useEffect(() => {
		...
		const handleReservationCreated = (reservation: Reservation) => {
			...
			if (isVoiceReservation(reservation)) {
				console.log(
					"Voice reservation detected, auto-conferencing:",
					reservation.sid,
				);
				...
				console.log(
					"Non-voice reservation, no conferencing needed:",
					reservation.sid,
				);
			}
		};
	...
	const smartAcceptReservation = useCallback(
		async (reservationSid: string) => {
			if (!worker) return;
			...
			if (isVoiceReservation(reservation)) {
				...
				if (reservation.status === "pending") {
					...
					if (incomingCall) {
						console.log(
							"Found incoming call, accepting call:",
					...
					if (relatedCall) {
						console.log(
							"Found related call, accepting call:",
							relatedCall.parameters.CallSid,
						);
	...
	const smartRejectReservation = useCallback(
		async (reservationSid: string) => {
			if (!worker) return;
			...
			if (isVoiceReservation(reservation)) {
				...
				if (reservation.status === "pending") {
					...
					if (incomingCall) {
						console.log(
							"Found incoming call, rejecting call:",
							incomingCall.parameters.CallSid,
						);
					...
					if (relatedCall) {
						console.log(
							"Found related call, rejecting call:",
							relatedCall.parameters.CallSid,
						);
	...
	const manualConference = useCallback(
		async (reservationSid: string) => {
	...
	const transferReservation = useCallback(
		async (reservationSid: string, targetWorkerSid: string) => {
			if (!worker) return;
	...
	const completeReservation = useCallback(
		async (reservationSid: string, attributes?: any) => {
			if (!worker) return;
		...
		return reservations
			.filter((reservation) => {
				...
				if (
					...
					console.log(
						"Hiding pending voice reservation without conference info:",
						reservation.sid,
					);
					return false;
				}
	...
	const orphanedCalls = useMemo(() => {
		...
		const filtered = activeCalls.filter((call) => {
			...
			const hasMatchingAcceptedReservation = reservations.some(
				(reservation) => {
			...
			if (hasMatchingAcceptedReservation) {
				console.log(
					"Call has matching accepted reservation, hiding:",
			...
			if (call.status() === Call.State.Ringing) {
				const hasPendingVoiceReservation = reservations.some(
					(reservation) =>
						isVoiceReservation(reservation) && reservation.status === "pending",
				);

				if (hasPendingVoiceReservation) {
					console.log(
						"Hiding ringing call because there is a pending voice reservation:",
						call.parameters.CallSid,
					);
				}
		...
		console.log(
			"Final orphaned calls:",
			filtered.map((call) => call.parameters.CallSid),
		);
		return filtered;
