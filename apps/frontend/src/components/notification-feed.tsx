...
const NotificationCenter = (props: Props) => {
	const { unreadNotifications, markAllNotificationsAsRead } =
		useNotifications();
		...
		<Popover>
			...
			<PopoverContent
				...
				style={{
					...
					maxHeight: 'min(-100px + 100vh, 800px)',
				}}
			>
		...
		>
			...
			<div className='flex flex-col items-start'>
				<h3 className='text-sm font-medium'>
					{notification?.from} approved your proposal
				</h3>
		...
		<div className='flex gap-3 p-3 border-b'>
			...
			<div className='flex-1 space-y-1.5'>
				<p className='font-bold text-base'>
					{notification.type === 'reservation_missed' &&
						'Engagement missed'}
					{notification.type === 'proposal_approved' &&
						'Proposal approved'}
					{notification.type === 'automate_alert' && 'Automate Alert'}
				</p>

				<p className='text-sm'>
					{notification.type === 'reservation_missed' &&
						`Missed an engagement from ${notification.resource_name}`}
					{notification.type === 'proposal_approved' &&
						`${notification.resource_name} has been approved by ${notification.from}`}
					{notification.type === 'automate_alert' &&
...
notificationHelperText.set('proposal_approved', {
	...
	description:
		'Expect a transaction for $48.64 in 3 days from Bird Rock Coffee Roasters.',
});
notificationHelperText.set('reservation_missed', {
	title: 'Engagement missed',
	description:
		'Your reservation with $48.64 from Bird Rock Coffee Roasters has been missed.',
});
