...
export const updateProposalSettings = createServerFn()
	.validator(
		({
			version,
			settings,
	...
	.handler(async ({ data: { version, settings } }) => {
...
export const updateNotification = createServerFn()
	.validator(
		(params: { id: string; notification: AppNotificationUpdate }) => params,
	)
	.handler(async ({ data: { id, notification } }) => {
		const supabase = createClient();
		const { error } = await supabase
			.from("notifications")
			.update(notification)
...
export const updatePinnedItem = createServerFn()
	.validator(({ id, item }: { id: string; item: PinnedItemUpdate }) => ({
		id,
		item,
	}))
...
export const updateProduct = createServerFn()
	.validator(({ id, product }: { id: string; product: ProductUpdate }) => ({
		id,
		product,
	}))
	.handler(async ({ data: { id, product } }) => {
		const supabase = createClient();
		const { error } = await supabase
			.from("products")
			.update(product)
 ...
 */
export const updateProposal = createServerFn()
	.validator(
		({
			id,
			proposal,
	...
	.handler<undefined>(async ({ data: { id, proposal } }) => {
		const supabase = createClient();
...
export const updateTicket = createServerFn()
	.validator(({ id, ticket }: { id: string; ticket: TicketUpdate }) => ({
		id,
		ticket,
	}))
	.handler(async ({ data: { id, ticket } }) => {
		const supabase = createClient();
		const { error } = await supabase
			.from("tickets")
			.update(ticket)
 ...
 * @param {TicketUpdate} ticket - The ticket you're wanting to update.
 */
export const upsertTickets = createServerFn()
	.validator(({ ticket }: { ticket: TicketInsert | TicketInsert[] }) => ({
		ticket,
	}))
 ...
 * @param {TicketUpdate} ticket - The ticket you're wanting to update.
 */
export const upsertTasks = createServerFn()
	.validator(({ task }: { task: TaskInsert | TaskInsert[] }) => ({ task }))
 ...
 */
export const updatePhase = createServerFn()
	.validator(({ id, phase }: { id: string; phase: PhaseUpdate }) => ({
		id,
		phase,
	}))
	.handler(async ({ data: { id, phase } }) => {
		const supabase = createClient();
...
export const upsertPhase = createServerFn()
	.validator(({ phase }: { phase: PhaseInsert | PhaseInsert[] }) => ({ phase }))
...
export const updateTask = createServerFn()
	.validator(({ id, task }: { id: string; task: TaskUpdate }) => ({ id, task }))
	.handler(async ({ data: { id, task } }) => {
		const supabase = createClient();
...
export const updateOrganization = createServerFn()
	...
	.handler(async ({ data: { id, organization } }) => {
		const supabase = createClient();
		const { error } = await supabase
			.from("organizations")
			.update(organization)
 ...
 */
export const updateOrganizationIntegration = createServerFn()
	.validator(
		({
			id,
			orgIntegration,
	...
	.handler(async ({ data: { id, orgIntegration } }) => {
		const supabase = await createClient();
		const { error } = await supabase
...
export const updateSection = createServerFn()
	.validator(({ id, section }: { id: string; section: SectionUpdate }) => ({
		id,
		section,
	}))
	.handler(async ({ data: { id, section } }) => {
		const supabase = createClient();
		const { error } = await supabase
			.from("sections")
			.update(section)
...
export const upsertSection = createServerFn()
	.validator(({ section }: { section: SectionInsert }) => ({ section }))
...
export const updateVersion = createServerFn()
	.validator(({ id, version }: { id: string; version: VersionUpdate }) => ({
		id,
		version,
	}))
...
export const updateUserMetadata = createServerFn()
	.validator(
		({
			data,
			user_metadata,
	...
	.handler(async ({ data: { data, user_metadata } }) => {
		const supabase = await createClient();
