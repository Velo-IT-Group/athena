...
export const lookupPhoneNumber = createServerFn().validator((from: string) =>
	from
).handler(async ({ data: from }) => {
	const client = await createClient();
	const { nationalFormat } = await client.lookups.v2.phoneNumbers(from)
		.fetch();
	...
	if (!contacts.length && companies.length === 1) {
		return {
			...
			territoryName: companies[0].territory?.name.split(" ").join("") ??
				"TeamA",
		};
	}
	...
	if (contacts.length > 1) {
		...
		if (
			...
			return {
				...
				territoryName: company?.territory?.name.split(" ").join("") ??
					"TeamA",
			};
		}
	...
	const territoryName = company?.territory && company?.territory?.name
