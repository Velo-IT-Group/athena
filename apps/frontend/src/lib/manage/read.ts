...
export const getCompanies = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		if (!response.ok || !countResponse.ok) {
			...
			throw new Error(
				(countResponse.statusText || response.statusText) +
					" " +
...
export const getCompanyNotesCount = createServerFn()
	.validator(
		({
			id,
			conditions,
	...
	.handler<{ count: number }>(async ({ data: { id, conditions } }) => {
...
export const getContactsCount = createServerFn()
	.validator((conditions?: Conditions<Contact>) => generateParams(conditions))
	.handler(async ({ data }): Promise<{ count: number }> => {
		...
		if (!countResponse.ok) {
			console.error(countResponse.statusText);
			throw new Error("Could not fetch contacts count...");
		}
...
export const getConfigurationsCount = createServerFn()
	...
	.handler<{ count: number }>(async ({ data: conditions }) => {
		...
		if (!countResponse.ok) {
			console.error(countResponse.statusText);
			throw new Error("Could not fetch configurations count...");
		}
...
export const getTicketNotes = createServerFn()
	...
	.handler<ServiceTicketNote[]>(async ({ data: { id, conditions } }) => {
		const response = await fetch(
...
export const getSystemMember = createServerFn()
	.validator(
		({
			id,
			conditions,
	...
	.handler(async ({ data }) => {
		const response = await fetch(
			`${env.VITE_CONNECT_WISE_URL}/system/members/${data.id}/${
				data.conditions || ""
			}`,
			{
				headers: baseHeaders,
...
export const getStatuses = createServerFn()
	.validator(
		({
			id,
			conditions,
	...
	.handler<BoardStatus[]>(async ({ data: { id, conditions } }) => {
		const response = await fetch(
			`${env.VITE_CONNECT_WISE_URL}/service/boards/${id}/statuses/${conditions}`,
			{
				headers: baseHeaders,
			},
		);
		...
		if (!response.ok) {
			console.error(response.statusText);
			throw Error(
				"Error fetching service board statuses... " +
					JSON.stringify(await response.json()),
				{
					cause: response.statusText,
				},
...
export const getBoardTypes = createServerFn()
	.validator(
		({
			...
			id: number;
			conditions?: Conditions<BoardType>;
		}) => ({
			id,
			conditions: generateParams(conditions),
		}),
...
export const getBoardItems = createServerFn()
	.validator(
		({
			...
			id: number;
			conditions?: Conditions<BoardItem>;
		}) => ({
			id,
			conditions: generateParams(conditions),
		}),
...
export const getTemplates = createServerFn()
	...
	.handler(async ({ data: conditions }): Promise<ProjectTemplate[]> => {
		...
		return templates.map((template) => {
			return {
				...template,
				workplan: workplans.find(
					(workplan) => workplan.templateId === template.id,
				),
			};
		});
	});
...
export const getWorkplan = createServerFn()
	.validator(
		(params: { id: number; conditions?: Conditions<ProjectWorkPlan> }) => ({
			id: params.id,
			conditions: generateParams(params.conditions),
		}),
	)
	.handler(async ({ data: { id, conditions } }) => {
		const response = await fetch(
...
export const getCatalogItems = createServerFn()
	...
	.handler(async ({ data: conditions }) => {
		...
		const bundles = catalogItems?.filter(
		...
		const mappedData: ExtendedCatalogItem[] = catalogItems.map((item) => {
			return {
				...item,
				bundledItems: bItems?.filter(
					(bItem) => bItem && bItem.parentCatalogItem.id === item.id,
				),
			};
		});
