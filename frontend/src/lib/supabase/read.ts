...
export type EngagementQueryOptions = {
...
export const getEngagementSummaryByPeriod = createServerFn()
	...
	.handler(async ({ data: options }) => {
		...
		const query = supabase.schema('reporting').from('call_summary_by_period').select();
		...
		if (error) {
			throw new Error('Error in getting call summary by period ' + error.message, {
				cause: error,
			});
		}
...
export const getEngagementReservations = createServerFn()
	...
	.handler(async ({ data: sid }) => {
		...
		if (error) {
			throw new Error('Error in getting engagement reservations ' + error.message, {
				cause: error,
			});
		}
...
export const getEngagements = createServerFn()
	...
	.handler(async ({ data: options }) => {
		...
		const query = supabase
			.schema('reporting')
			.from('engagements')
			.select('*, reservations:engagement_reservations(*)', { count: 'exact' });
...
export const getEngagement = createServerFn()
	...
	.handler(async ({ data: id }) => {
		...
		const { data, error } = await supabase.schema('reporting').from('engagements').select().eq('id', id).single();

		if (error) {
			throw new Error('Error in getting engagement for id ' + id + ' ' + error.message, {
				cause: error,
			});
		}
...
export const getPinnedItems = createServerFn().handler(async ({ data }) => {
	...
	const { data: pinnedItems, error } = await supabase.from('pinned_items').select();
...
export const getPinnedItem = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const { data: pinnedItem, error } = await supabase.from('pinned_items').select().match(data).single();
...
export const getProposals = createServerFn()
	...
	.handler(async ({ data }) => {
		...
		const proposalsQuery = supabase.from('proposals').select('*', { count: 'exact' });
...
export const getProfilePhoneNumber = createServerFn()
	...
	.handler(async ({ data: id }) => {
		...
		const { data, error } = await supabase.from('phone_numbers').select().eq('assigned_to', id).single();
...
export const getProposalTotals = createServerFn()
	...
	.handler(async ({ data: { version, id } }) => {
		...
		if (error) {
			throw new Error('Error in getting proposal totals ' + error.message, { cause: error });
		}
...
export const getTemplate = createServerFn()
	...
	.handler(async ({ data: id }) => {
		...
		const { data, error } = await supabase
			...
			.select('*, phases:phase_templates(*, tickets:ticket_templates(*, tasks:task_templates(*)))')
			.eq('id', id)
		...
		if (error) {
			throw new Error('Error in getting template ' + id + ' ' + error.message, { cause: error });
		}
...
export const getProposalFollowers = createServerFn()
	...
	.handler(async ({ data: id }) => {
		...
		const { data, error } = await supabase.from('proposal_followers').select('...user_id(*)').eq('proposal_id', id);
...
export const getPhases = createServerFn()
	.validator(({ versionId, proposalId }: { versionId: string; proposalId: string }) => ({
		versionId,
		proposalId,
	}))
...
export const getStorageFile = createServerFn()
	...
	.handler(async ({ data: { bucketName, path } }) => {
		...
		const { data, error } = await supabase.storage.from(bucketName).createSignedUrl(path ?? '', DAY_IN_MS / 12);
...
export const getProposalSettings = createServerFn()
	.validator(({ id, version }: { id: string; version: string }) => ({ id, version }))
	.handler(async ({ data: { id, version } }) => {
		const supabase = createClient();
