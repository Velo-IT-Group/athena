...
export const useConference = ({ sid }: Props) => {
	...
	const handleParticipantUpdate = useMutation({
		...
		onMutate: async ({ participantSid, options }) => {
			...
			const previousItems = queryClient.getQueryData<ParticipantInstance[]>(queryKey) ?? [];
			...
			const updatedItem = previousItems?.find((item) => item.callSid === participantSid);
	...
	const handleParticipantCreation = useMutation({
		mutationFn: ({ options }: { options: ParticipantListInstanceCreateOptions }) =>
			createParticipant({
				data: { sid, options },
			}),
		onMutate: async ({ options }) => {
			...
			newItems = [...previousItems, options as unknown as ParticipantInstance];
	...
	return {
		participants: data ?? [],
		updateParticipant: handleParticipantUpdate,
		createParticipant: handleParticipantCreation,
