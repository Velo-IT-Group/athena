...
export const useTicket = ({
	...
	const updateProposalTotals = (phases: NestedPhase[]) => {
		...
		const laborCost = phases.reduce(
			(acc, p) => acc + (p.hours ?? 0) * (previousTotals.labor_rate ?? 0),
			0
		);
	...
	const updatePhaseHours = (updatedTickets: NestedTicket[]) => {
		const phases = queryClient.getQueryData<NestedPhase[]>(
			phasesQuery.queryKey
		);
		...
		const updatePhase: NestedPhase = {
			...phase,
			hours: updatedTickets.reduce(
				(acc, t) => acc + (t.budget_hours ?? 0),
				0
			),
		};

		const updatedPhases = phases?.map((p) =>
			p.id === phaseId ? updatePhase : p
		);
	...
	const { mutate: handleTicketInsert } = useMutation({
		...
		onMutate: async ({ ticket, tasks }) =>
			// @ts-ignore
			await addCacheItem<NestedTicket>(queryClient, queryKey, {
				...ticket,
				tasks,
			}),
		onError: (err, newPhase, context) => {
	...
	const { mutate: handleTicketDeletion } = useMutation({
		...
		onMutate: async ({ id }) =>
			await deleteCacheItem<NestedTicket>(
				queryClient,
				queryKey,
				(item) => item.id === id
			),
	...
	const { mutate: handleTicketDuplication } = useMutation({
		...
		onMutate: async ({ id }) => {
			const previousTickets =
				queryClient.getQueryData<NestedTicket[]>(queryKey) ?? [];
		...
		onError: (err, newPhase, context) => {
			queryClient.setQueryData(
				queryKey,
				context?.previousItems ?? ([] as NestedTicket[])
			);
		},
		onSettled: async () => {
			await queryClient.invalidateQueries({
