...
export function flattenObject<T extends CamelCaseObject>(
	...
	type FlattenResult<T> = T extends { name: infer U } ? U
		: SnakeCaseObject<T>;
...
export const calculateTotals = (
	...
	const ticketHours = phases && phases.length
	...
	const productTotal: number = products
		.filter((p) =>
			!p.recurring_flag || p.recurring_bill_cycle !== 2 ||
			p.parent !== null
		)
		.reduce((accumulator, currentValue) => {
			const price: number | null = currentValue.product_class === "Bundle"
	...
	const productCost: number = products
		...
		.reduce((accumulator, currentValue) => {
			const cost: number | null = currentValue.product_class === "Bundle"
