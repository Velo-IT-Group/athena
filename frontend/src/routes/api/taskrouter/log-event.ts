...
export const ServerRoute = createServerFileRoute("/api/taskrouter/log-event").methods({
    POST: async ({ request }) => {
        ...
        await Promise.all(
            response.map((event) =>
                supabase.from("taskrouter_events").insert({
                    ...
                    payload: {
                        ...event.data.payload,
                        task_attributes: JSON.parse(
                            event.data.payload.task_attributes,
                        ),
                        worker_attributes: JSON.parse(
                            event.data.payload.worker_attributes,
                        ),
                    },
