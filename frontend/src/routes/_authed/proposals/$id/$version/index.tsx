...
import { getStorageFilesQuery } from '@/lib/supabase/api';
import { getPhases, getProducts, getProposal, getProposalSettings, getStorageFiles } from '@/lib/supabase/read';
...
export const Route = createFileRoute('/_authed/proposals/$id/$version/')({
	...
	loader: async ({ params }) => {
		...
		const ticket = await getTicket({ data: { id: proposal?.service_ticket ?? 0 } });

		if (!proposal) throw redirect({ to: '/proposals' });
...
function RouteComponent() {
	const { proposal: initalProposal, settings: initalSettings, ticket: initalTicket } = Route.useLoaderData();
	...
	const {
		...
		queryKey: ['proposals', id, version, 'files'],
		queryFn: () => getStorageFiles({ data: { bucketName: 'attachments', path: `proposals/${id}/${version}` } }),
	...
	} = useProposal({ id, version, initialData: initalProposal });

	const { data: ticket } = useQuery({
		queryKey: ['tickets', proposal?.service_ticket],
		queryFn: () => getTicket({ data: { id: proposal?.service_ticket ?? 0 } }),
		initialData: initalTicket,
		enabled: !!proposal?.service_ticket,
	});

	if (!proposalSettings) return <div></div>;
