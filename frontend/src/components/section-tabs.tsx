...
const SectionTabs = ({ params }: Props) => {
	const [{ data: initialData }, { data: initialProducts }] =
		useSuspenseQueries({
			queries: [
				getSectionsQuery(params.id, params.version),
				getProductsQuery(params.version),
			],
		});
	...
	const [columns, setColumns] =
		useState<Record<UniqueIdentifier, NestedProduct[]>>(groupedSections);
	...
	return (
		<div className='flex flex-col items-start w-full flex-shink p-3 space-y-3 px-6'>
					...
					onValueChange={(value) => {
						...
						if (areSectionsSame) {
							...
							const phasesToUpdate =
								new Map<string, PhaseUpdate>();
							newColumns.forEach(([key, value], order) => {
								const currentPhase = orderedSections.find(
									(phase) => phase.id === key
								);
