import { Computer, LayoutPanelLeft, Monitor, User } from 'lucide-react';
import {
	Breadcrumb,
	BreadcrumbItem,
	BreadcrumbLink,
...
export function SettingsDialog({ session }: { session: Session }) {
	...
	const [selectedTab, setSelectedTab] = useState<(typeof data.nav)[number]['name']>(data.nav[0].name);
			...
			<SidebarProvider className='items-start'>
				...
				<main className='flex h-[480px] flex-1 flex-col overflow-hidden'>
					...
					<div className='flex flex-1 flex-col gap-4 overflow-y-auto p-4 pt-0'>
						{selectedTab === 'Profile' && <SettingsDialogProfileForm />}

						{selectedTab === 'Features' && (
							<form className='flex items-center justify-between'>
								<div>
									<h4 className='font-medium'>Hide Queue Status</h4>
									<p className='text-sm text-muted-foreground'>
										Display queue status in right hand sidebar.
									</p>
								</div>
						...
						{selectedTab === 'Display' && <SettingsDialogDisplayForm />}
					</div>
				</main>
			</SidebarProvider>
		</DialogContent>
