...
export function FilterOperatorController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
...
function FilterOperatorOptionController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
	const filter = column.getFilterValue() as FilterModel<'option', TData>;
	const filterDetails = optionFilterDetails[filter.operator];

	const relatedFilters = Object.values(optionFilterDetails).filter((o) => o.target === filterDetails.target);
...
function FilterOperatorMultiOptionController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
	...
	const relatedFilters = Object.values(multiOptionFilterDetails).filter((o) => o.target === filterDetails.target);
...
function FilterOperatorDateController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
	const filter = column.getFilterValue() as FilterModel<'date', TData>;
	const filterDetails = dateFilterDetails[filter.operator];

	const relatedFilters = Object.values(dateFilterDetails).filter((o) => o.target === filterDetails.target);
...
export function FilterOperatorTextController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
	const filter = column.getFilterValue() as FilterModel<'text', TData>;
	const filterDetails = textFilterDetails[filter.operator];

	const relatedFilters = Object.values(textFilterDetails).filter((o) => o.target === filterDetails.target);
...
function FilterOperatorNumberController<TData>({ column, closeController }: FilterOperatorControllerProps<TData>) {
...
export function FilterValueDateDisplay<TData, TValue>({ column }: FilterValueDisplayProps<TData, TValue>) {
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'date', TData>) : undefined;
...
export function FilterValueTextDisplay<TData, TValue>({ column }: FilterValueDisplayProps<TData, TValue>) {
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'text', TData>) : undefined;
...
export function FilterValueNumberDisplay<TData, TValue>({
	...
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'number', TData>) : undefined;
...
export function FilterValueOptionController<TData, TValue>({
	...
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'option', TData>) : undefined;
	...
	const optionsCount: Record<ColumnOption['value'], number> = columnVals.reduce((acc, curr) => {
...
export function FilterValueDateController<TData, TValue>({ column }: ProperFilterValueMenuProps<TData, TValue>) {
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'date', TData>) : undefined;
...
export function FilterValueTextController<TData, TValue>({ column }: ProperFilterValueMenuProps<TData, TValue>) {
	const filter = column.getFilterValue() ? (column.getFilterValue() as FilterModel<'text', TData>) : undefined;
...
export function FilterValueNumberController<TData, TValue>({
	...
	const slider = {
		value: inputValues.map((val) => (val === '' || val === `${cappedMax}+` ? cappedMax : Number.parseInt(val, 10))),
