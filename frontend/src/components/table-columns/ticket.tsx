...
export const columns: ColumnDef<ServiceTicket>[] = [
	{
		...
		cell: ({ row }) => {
			...
			return (
				<div className='w-[80px] justify-start px-0 text-sm group flex items-center'>
					<Link
						to='/tickets/$id'
						params={{ id }}
					>
					...
					<div
						...
						className='flex items-center justify-between gap-2 pl-4 pr-1.5 group-hover:opacity-100 opacity-0 transition-all duration-200 ease-in-out w-0 group-hover:w-5 h-5'
					>
						<button
							type='button'
							aria-busy='false'
							...
							data-state='closed'
							className='sc-gnqCJb iJhEYM'
						>
							<div
								data-is-small-ghost='true'
								className='sc-kDXjwZ jFztyZ'
							>
								<MessagesSquare />
							</div>
						</button>
			...
			return (
				<div className='flex items-center space-x-1.5'>
					<Circle
						className={cn(
							'stroke-none',
							row?.original?.priority?.id === 6 &&
								'fill-[#DC3221]',
							row?.original?.priority?.id === 7 &&
								'fill-[#029E73]',
							row?.original?.priority?.id === 8 &&
								'fill-[#56B4E9]'
						)}
					/>
	...
	},
	{
		...
		filterFn: (row, id, value) => {
			...
			return referenceRow && referenceRow.id
				? value.includes(String(referenceRow.id))
				: false;
		},
		meta: {
		...
		cell: ({ row }) => {
			...
			return (
				<ContextMenu>
					...
					<ContextMenuContent>
						<ContextMenuGroup title='Actions'>
							{reference?.id && (
				...
				<Button
					...
					onClick={(e) => {
						if (e.altKey) {
							...
							searchParams.set(
								'companyId',
								String(row.original.company!.id)
							);
